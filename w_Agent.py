...
def main():
    ...
    wandb.run.name = f'gMixup_{config.gMixup}_GD_{config.GD}_Dataset_{config.dataset_name}_testModel_{config.model}'
...
#use your wandb keys here
wandb.login(key="your keys")


#有两个要看的点，跑一圈四个：要不要mixup，要不要GD。四个组合，即为：mixup+GD，mixup+noGD，no mixup+GD，no mixup+noGD
...
wandb_id = wandb.sweep(sweep_config, project='GD_Mixup')
wandb.agent(wandb_id, main, count=4)
...
#         'loader_bs': {'value': 128},
#         'mixup_fixed': {'value': 0.5},
#         'lam_range': {'value': "[0.005, 0.01]"},
#         'aug_ratio': {'value': 0.15},
#         'aug_num': {'value': 10},
#         'batch_size': {'value': 32},
#         'epochs': {'value': 200},
#         'init': {'value': 'real'},
#         'stru_discrete': {'value': 1},
#         'lr_adj': {'value': 0.0001},
#         'lr_feat': {'value': 0.0001},
#         'lr_model': {'value': 0.001},
#         'ipc': {'value': 0},
#         'reduction_rate': {'value': 0.1},
#         'pooling': {'value': 'mean'},
#         'net_norm': {'value': 'none'}
#     }
# }