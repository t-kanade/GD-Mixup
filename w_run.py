...
def main(args):
    ...
    compare_Agent = compare(args, device, compare_data, args.epochs)
...
wandb.init(project='GD_Mixup', config=config)
...
args.mixup_fixed = config.mixup_fixed


wandb.run.name = f'Test_gMixup_{config.gMixup}_GD_{config.GD}_Dataset_{config.dataset_name}_testModel_{config.model}_mixFix_{config.mixup_fixed}'

#use random to generate the seed_list
# seed_list = random.sample(range(1, 10000), 100)
